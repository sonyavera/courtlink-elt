...
with pg_client._connect() as conn, conn.cursor() as cur:
    ...
    cur.execute(f"TRUNCATE table {pg_schema}.reservations_raw")
    cur.execute(f"TRUNCATE table {pg_schema}.reservations_raw_stg")
    cur.execute(
        f"""UPDATE {pg_schema}.elt_watermarks
            SET last_loaded_at = %s, 
                last_record_created_at = %s
            WHERE source_name in ('reservations_event', 'reservations_non_event')
        """,
        (watermark, watermark),
    )
