...
if len(args) and args[0] == "test":
    watermark = (datetime.today() - timedelta(weeks=8)).date()
...
with pg_client._connect() as conn, conn.cursor() as cur:
    ...
    cur.execute(f"TRUNCATE table {pg_schema}.event_summary_raw")
    cur.execute(f"TRUNCATE table {pg_schema}.event_summary_raw_stg")
    cur.execute(
        f"""UPDATE {pg_schema}.elt_watermarks
            SET last_loaded_at = %s, 
                last_record_created_at = %s
            WHERE source_name = 'event_summaries'
        """,
        (watermark, watermark),
    )
